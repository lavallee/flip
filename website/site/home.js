/* Home page: hydrate the hero record, the four enforcement rules, and the
   coordinates — all from the data the build generated by running the real CLI.
...
(function () {
  ...
  // -- hero: the claim page that earned its status --------------------------
  var heroHost = document.getElementById("hero-record");
  var verified = step("verified");
  if (heroHost && verified) {
    heroHost.textContent = "";
    heroHost.appendChild(kit.record({
      path: verified.record.path,
      label: "· after " + verified.commands[0],
      text: verified.record.text,
      caption: "status: verified, and independent_corroboration recomputed from the "
        + "sources actually present — not trusted from the page."
    }));
  }
